...
describe("Frontmatter completion", function()
  describe("Enum fields", function()
    ...
    it("should offer every registered backend in agent enum values", function()
      ...
      assert.are.equal("pi", items[5].word)
    end)
    ...
    it("should offer the CLI default before pinned effort levels", function()
      local items = frontmatter_provider.get_enum_values("effort")
      assert.are.equal(6, #items)
      assert.are.equal("default", items[1].word)
      assert.are.equal("low", items[2].word)
      assert.are.equal("max", items[6].word)
    end)
