...
describe("Frontmatter completion", function()
  describe("Enum fields", function()
    ...
    it("should list claude, codex, and grok for agent field", function()
      local items = frontmatter_provider.get_enum_values("agent")
      local words = vim.tbl_map(function(item)
        return item.word
      end, items)
      assert.is_true(vim.tbl_contains(words, "claude"))
      assert.is_true(vim.tbl_contains(words, "codex"))
      assert.is_true(vim.tbl_contains(words, "grok"), "agent completion must include grok")
      assert.are.equal(3, #items)
    end)

    it("should detect agent field trigger and return grok among candidates", function()
      local ctx = frontmatter_source.get_trigger_context("agent: ", 7)
      assert.is_not_nil(ctx)
      assert.are.equal("frontmatter_enum", ctx.trigger)
      assert.are.equal("agent", ctx.field)
      local items = frontmatter_source.get_candidates_sync(ctx)
      local words = vim.tbl_map(function(item)
        return item.word
      end, items)
      assert.is_true(vim.tbl_contains(words, "grok"))
    end)

    it("should filter candidates by query", function()
      local ctx = frontmatter_source.get_trigger_context("permissions_mode: accept", 24)
      local items = frontmatter_source.get_candidates_sync(ctx)
