-- ADR 009 C2 against the real copilot CLI: a tool call is drawn in claude's shape.
--
-- This is the behaviour change P1 made deliberately -- copilot used to draw its own header when a
-- tool started, where claude draws one with the result -- so it is also the one a future decoder
-- edit can undo without any unit test noticing. `conformance/renderer_parity_spec.lua` pins the
-- rendering given canonical events and `stream_fixtures_spec.lua` replays a capture through the
-- decoder; neither can tell that today's copilot still pairs `tool.execution_start` with
-- `tool.execution_complete` by `toolCallId` the way the capture did. Only a real turn can.
--
...
describe("E2E: copilot renders a tool call in claude's shape", function()
  ...
  before_each(function()
    nvim_instance = helper.spawn_backend_instance("copilot")
  end)
