describe("vibing.infrastructure.file_finder.fd_command", function()
  local fd_command = require("vibing.infrastructure.file_finder.fd_command")

  local function skip_if_unsupported(finder)
    if not finder:supports_platform() then
      pending("fd command not available on this platform")
      return true
    end
    return false
  end
  ...
  describe("new", function()
    it("should create instance with correct name", function()
      ...
      assert.equals("fd_command", finder.name)
    end)

    it("should accept mtime_days option", function()
      local finder = fd_command:new({ mtime_days = 7 })
      assert.equals(7, finder.mtime_days)
    end)
  end)

  describe("supports_platform", function()
    it("should return boolean indicating fd availability", function()
      local finder = fd_command:new()
