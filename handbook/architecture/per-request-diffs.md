...
`find_other_active_for_session` does — grok registers no `chat_bufnr` (see `features.md`), so two
grok streams would compare `nil` against `nil` and never see each other.
