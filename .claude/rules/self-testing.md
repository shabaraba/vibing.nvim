...
- **Agent behavior evals are separate from E2E.** `pnpm run test:eval` (`tests/evals/`) checks the
...
- **3-try auto-fix rule.** After implementing a feature, run `pnpm run test:e2e`. On failure:
