 ...
 *
 * The hook is installed on `agent.beforeToolCall`, which is per **Agent** and not per tool, so
 * `web_fetch` and `web_search` — registered by this same extension — are gated by it exactly as
 * Pi's own `bash` is. There is no path by which a tool this extension adds escapes the gate the
 * same extension installs.
 *
...
/** The `tool_call` handler Pi calls before every tool, built-in or registered here. */
export async function decide(event: any, ctx: any): Promise<Verdict | undefined> {
