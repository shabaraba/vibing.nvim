...
---
---「起動時点でアクティブなリクエストは存在しない」が言えるのは **自プロセスについてだけ**。
---同じworktreeを別のNeovimが既に開いた状態でこちらが起動することはあり、その実行中のrefを
---消してはいけないので、実際の可否判断は `sweep_refs` が持っている（他プロセスの生存確認と
---年齢の二段構え）。ここを「無条件に消してよい」と読んで向こうのチェックを外さないこと。
---@param cwd string|nil
...
M._INTERNAL_TOOLS_THAT_MUTATE = INTERNAL_TOOLS_THAT_MUTATE
