...
local function process_done(bufnr)
  ...
  -- 停止理由は1回だけ読んで3つの判断すべてに使う。この関数の中で理由が書き換わることはないので、
  -- 2度読んで食い違う心配はない
  ...
  --
  -- 例外も分岐2と同じ。自力では抜けられない止まり方をしているなら、配り直しは助けにならない:
  -- 承認待ちも質問も未送信の `## User` セクションとして残るので、枠が空いても `flush` は
  -- 「下書きあり」として配達を断る。そのチャットは二度と走らず完了イベントも二度と来ないので、
  -- ここで保留すると親は詰まったことを永久に知らない
  if held and not stop_reason then
    return
  end
  ...
  if is_waiting_on_others(bufnr) and not stop_reason then
    return
  end
